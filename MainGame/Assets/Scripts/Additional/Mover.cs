...
public abstract class Mover : Fighter
{
    ...
    {
        // Переназначение вектора движения
        moveVector = new Vector3(input.x * Xspeed, input.y * Yspeed, 0);      

        // Отражение спрайта в зависимости от движения влево или вправо
        if (moveVector.x < 0)
            transform.localScale = Vector3.one;
        else if (moveVector.x > 0)
            transform.localScale = new Vector3(-1,1,1);

        // Добавить вектор отталкивания
        moveVector += pushDirection;
        // Уменьшение силы отталкивания каждый кадр, основываясь на скорости восстановления от удара
